...
#  		3. Creates View for traffic analysis                             #
#  		4. Creates Weekly Data Mart for paid traffic tracking with Event #
#          trigger                                                       #
#  		5. Creates View for top landing pages                            #
#  Notes:                                                                #
# ---------------------------------------------------------------------- #
...
DROP VIEW IF EXISTS cross_sell_yearly;

CREATE VIEW cross_sell_yearly AS
(
select
...
# ---------------------------------------------------------------------- #
#  3. Top traffic sources and bounced sessions                           #
# ---------------------------------------------------------------------- #

DROP VIEW IF EXISTS top_traffic_sources;

CREATE VIEW top_traffic_sources AS

select
	session_created_at_yr as 'Year',
	utm_source as 'UTM source',
    utm_campaign as 'UTM campaign name',
    http_referer as 'URL of HTTP referer',
    
    count(distinct website_session_id) as 'Number of user sessions',
...
and date(session_created_at) > '2014-08-17'
and website_session_id <> '472880' -- manually inserted session
group by 1,2,3,4
order by 1,5 desc;

select * from top_traffic_sources;
